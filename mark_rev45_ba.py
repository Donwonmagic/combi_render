# mark_rev45_ba.py -- rev 45.  THE BEFORE | AFTER | PHOTOGRAPH SHEET.
#
# THIS IS A QUESTION, and it is the SECOND question figure this revision.
#
# WHY THERE ARE TWO.  `mark_rev45_q.py` is rev 44b's, and it is right about the
# thing it is right about: rev 36 shipped a figure with five mark classes and
# got back "i don't understand what is being asked", then shipped one crop, one
# circle, one sentence and got the most valuable answer in ten revisions.  ITS
# RULE STANDS -- if he does not understand the question, the FIGURE is the
# defect, not him -- and this file obeys it: NO boxes, NO arrows, NO leader
# lines, one sentence per row.  The crop is the pointer.
#
# What this file adds is not a mark class.  It is two more PICTURES per row.
# Rev 44b's figure cannot show that anything changed, because it crops one
# render; and the rev-45 brief sec.0's three candidate causes are all about
# evidence rather than measurement.  Three pictures kill all three at once.
#
...
#
# MARK CLASSES -- there is ONE, and that is the point.
#   [L] PANEL LABEL   which build, or which photograph, this cell is.
# There is no identification box.  An earlier draft of this file had one and it
# was removed on rev 44b's authority: the crop already isolates the feature, so
# a box round the crop points at nothing the crop did not already point at, and
# every extra mark class is a chance for the figure to be the defect.
#
...
for r, (cw, rh, qlines) in zip(ROWS, rows_h):
    ...
    for i, (lbl, key, box) in enumerate(r["cells"]):
        ...
        dr.rectangle([ox - 1, oy - 1, ox + crop.width, oy + crop.height],
                     outline=(206, 202, 194), width=1)
        # [L] PANEL LABEL
