# NEXT CONTEXT PROMPT — rev 26

Please act as my expert. Continue the Señor Tacombi combi build. **Twenty-five
...
`-rev20`, `-rev21`, `-rev22`, `-rev23`, `-rev24`, then
**`/areas/tacombi-combi-3d-rev25.md`** (these are SEPARATE FILES; each
...
See §1 below. **THIRTEEN bundle lines now, and the rev14b line is a `fetch`.**
rev 18 opened with rev 17 missing and rev 19 with rev 18 missing; **rev 20, 21,
22, 23, 24 and 25 all opened CLEAN** — do not assume either way, check.
...
MOVED SINCE REV 23.** Read §10.59.

## Step 4 — read, in this order
`STATE.md` → `SPEC.md` §10, then §10.9 through §10.69 → this file →
...
work-item-2 brief.
**§10.68–69 are rev 25's: §10.68 REFUTES rev 24's `_ZB_AUTH` brief, finds the
committed artwork was FOURTEEN REVISIONS OLD, and re-bakes it; §10.69 takes
`_RETIRED_VALUES` from 5 rows to 15 and states the guard's re-expression
ceiling.**
...
its own briefing; rev 24 REFUTED ITS OWN WORK ITEM 2 OUTRIGHT and its new guard
was wrong TWICE before it was right; **rev 25 refuted its own work item 2's brief
AND refuted a figure inside a comment it was writing at that moment.**
...
at 0.0 mm. `folk_gen.composition()` RUNS. THE BAKE FRAME IS PARSED AND THE
ARTWORK IS RE-BAKED. THE HERO PHOTOGRAPHS BOTH THE CURRENT MESH AND THE CURRENT
ARTWORK. `COUNTERTAN`'s ~59 % pedestal IS THE OLDEST UNDONE ITEM — six revisions
on the list and NOT touched in rev 25.**
...
CURRENT artwork** — do not re-shoot unless something moves.
...
**NEW, rev 25 and now settled:**
**EVERY constant in `folk_gen.py` is PARSED and every parse RAISES** — never
re-type one back in. `_ceval` is deliberately tiny; extend it only for node
types a real definition uses.
**The committed `tex/*.png` are CURRENT as of rev 25** — `swirl.png`
`4ee4e09e`, `swirl_b.png` `d2015971`, `nose.png` `b31ea156` (unchanged, the nose
bake is independent of the flank frame). The old "baked in the STALE frame"
warning is **DISCHARGED**.
**The bake is DETERMINISTIC** (fixed seeds 196301 / 771963; two processes give
identical md5s) — so a bake diff is always provenance, never noise.
**`_DOOR_TOP_AUTH` is HELD at the authored 1.8140** so `DOOR_H` is bit-identical
across the rev-25 bake. **Do not change it to 1.8098 without a measurement.**
**Do NOT add a `_RETIRED_VALUES` row for the ±2.145 bumper faces** — no §10
entry retires either value, so a row would assert a retirement that does not
exist.
...
* **A claim in prose is not a guard — INCLUDING WHEN THE PROSE IS INSIDE THE
  GUARD, AND INCLUDING WHEN YOU ARE WRITING IT RIGHT NOW.** rev 25 wrote "within
  1 mm" into a comment and the print refuted it at 4.2 mm in the same minute.
  *(EXTENDED, rev 25)*
...
* **AN INFERENCE THAT LOOKS LIKE A CORRECTION IS STILL AN INFERENCE.**
* **A GUARD THAT IS RIGHT FOR THE WRONG REASON IS NOT A GUARD.**
...
* **A RETIRED VALUE RE-EXPRESSED IN ANOTHER FORM IS INVISIBLE TO A SUBSTRING
  GUARD** — edges for widths, mm/m for m/m, degrees for slope. §1.1 carried the
  retired bay taper as EDGE PAIRS three lines above the strike that retired the
  width form. The answer is another ROW, never a looser rule. *(NEW, rev 25)*
* **A MAGNITUDE CAN BE EXACTLY RIGHT AND THE CONCLUSION STILL WRONG. WEIGHT AN
  ERROR BY WHAT IT ACTUALLY TOUCHES.** `_ZB_AUTH`'s 76.222 mm reproduced to the
  micron and carried ZERO ink; the defect that mattered was 19 mm somewhere
  nobody had looked. *(NEW, rev 25)*
* **WHEN A CONTROL FAILS, THE CONTROL IS THE FINDING — but check the control's
  own premise first.** rev 25's re-bake control failed; determinism was verified
  in two processes BEFORE the failure was interpreted, and only then did a bisect
  show the artwork was fourteen revisions old. *(NEW, rev 25)*
...
* **A SUBAGENT'S FINDING IS A CLAIM, NOT A MEASUREMENT.** rev 25 ran two and
  verified every load-bearing claim by hand; four of one agent's candidates were
  refuted or mislocated.
...
* **IF A DERIVED CONSTANT IS YOUR FALSIFICATION LEVER, WORK OUT WHAT VALUE
  REPRODUCES THE OLD STATE.** rev 25's landed on −0.005250 against §10.62's
  independently derived −0.0053 — **0.050 mm apart, from two unrelated routes.**
...
* A single shell command is killed at 10 minutes. This box has 2 cores — spawn
  ...
  a valid file — let the SEAM CHECK adjudicate it, not the file opening cleanly.**
...
* **The device bridge may drop mid-transfer.** Try the direct call FIRST — rev
  25 moved thirteen bundles in ONE call — and fall back to `split` + parallel
  single-file calls + **md5 against my disk** only on failure. `device_bash`
  cannot `rm`. **Split the OUTBOUND batch** — rev 23's 19.7 MB batch timed out
  at 60 s while five files plus the tarball alone went straight through.
...
git pull --ff-only ../tacombi_rev25_incremental.bundle HEAD      # -> see §7
```
...
grep -c 'T1_CTAN_NOISOLATE' shader_solve.py             # 2   ANCESTOR rev 24
...
covered by the eight rev-25 content greps instead.**
...
**0.516 0.515 0.516**.
**EVERY GEOMETRY FIGURE IS IDENTICAL TO REV 23's. NO GEOMETRY MOVED IN REV 24 OR
REV 25.** What moved in rev 25 is the ARTWORK.

## 3. What rev 25 changed — `HANDOFF_rev25.md` has the full account
- **`folk_gen.py`** — every remaining re-typed constant is now PARSED from the
  module that owns it, via a deliberately tiny `_ceval` over `t1_shell`'s
  constant graph and `t1_core`'s `ZB` knots. `DOOR_X0` is expressed in terms of
  `BAYS[0][1]`; `T1_BPILLAR` moves the ART frame with the geometry; every parse
  RAISES rather than falling back.
- **`tex/swirl.png`, `tex/swirl_b.png`** — **RE-BAKED**, the first time since
  rev 11. `tex/nose.png` UNCHANGED.
- **`verify.py`** — `_RETIRED_VALUES` 5 rows → 15.
- **`SPEC.md`** — §10.68, §10.69, the change log, and **twelve** retired values
  struck across §1.1, §2, §2.3, §2.5, §3, §6 and §9.
- **`rev25_hero34f.png`** — 4800×3200, worst seam z 1.91.
- **`CREAM`, `COUNTERTAN`, `RED`, the rake, the roof and ALL GEOMETRY UNCHANGED.**

**Things you must not silently undo — `HANDOFF_rev25.md` §4**, and rev 24's §4,
rev 23's §4, rev 22's §3, rev 21's §4, rev 20's §4, rev 19's §4 and rev 18's §4
all still stand in full.

## 4. Still open
See `HANDOFF_rev25.md` §5 for the full list. The ones that matter:
- **`COUNTERTAN`'s ~59 % pedestal** — dust, wear, fade, coat+spec,
  interreflection and occlusion ALL excluded. **STILL UNIDENTIFIED**, and it is
  now the oldest undone item at six revisions. `k` is 40 % larger than believed —
  **re-derive, never carry the old secant gain.**
...
- **`_DOOR_TOP_AUTH`** — authored 1.8140 against the outline's top-run mean
  1.80980. **4.2 mm, NEW in rev 25, and neither value is measured against a
  photograph.**
- **Bumper faces `±2.145` (SPEC `:191`) vs `X_BUMP_F/R = ±2.140`** — a real 5 mm
  drift with **no retiring entry anywhere**. Not a guard row until one exists.
- **`X_NOSE` and `X_TAIL` are parsed in `folk_gen` and never LOADED** (AST
  census: 1 Store, 0 Loads each). Harmless; verified, not fixed.
- `PLATE_W = 0.3300` has no provenance. `probe_rev16.py:90` prints `xa` vs `xa`.

## 5. FIRST QUESTION FOR THE OWNER
**No decision is outstanding with me.** rev 25 asked no photograph question — it
was a measurement, bake and hygiene revision and had none a photograph could
answer.
...
1. **`COUNTERTAN`'s ~59 % pedestal** — name the next suspect and ablate it.
   Oldest undone item, six revisions on the list. **Re-derive `k` first.**
2. **`_DOOR_TOP_AUTH`** — settle the 4.2 mm with a measurement, then re-bake if
   it moves. The only thing rev 25 left half-settled.
3. **The ±2.145 / ±2.140 bumper drift** — establish which is right, retire one.
4. Tail-lamp material slot; `Senor`'s letterforms; `SCR`'s +80 mm.
5. A hero ONLY if geometry or artwork moves. `rev25_hero34f.png` currently
   photographs both.
...
gone wrong in ELEVEN revisions during handoff assembly** — a truncated console
tail, a stale hash, five wrong counts, a `grep -c` that counted LINES rather than
