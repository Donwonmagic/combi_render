...
| 28 | **NEW, rev 44 — HIS REPORT, CONFIRMED.** The door's shut line **merges into the wheel arch** behind the front wheel instead of turning a corner and running clear of it | **open.** `DOOR_ARCH_G` holds the outline only **24.4 mm** from the arch circle, so at any render resolution the two lines read as ONE curve — the shut line simply bends into the arch and nothing continues down behind the wheel. The reference (`ref_nolita_doorshut.jpg`) shows a **clean corner** and a roughly **flat** bottom run with a clear band of body metal between it and the arch, order **~90 mm**. Measured clearance to the body's lower edge is also strongly asymmetric: **26 mm at the front corner against 157 mm at the rear**. **NOT fixed here:** `DOOR_ARCH_G` was set by §10.100.4's fixed-point solve to land on rev 41's own value and is armed by a guard; raising it moves the whole bottom outline and therefore finding 1's art frame. It needs a *measured* clearance, not an eyeballed one. Evidence: `rev44_door_rear_corner.png`. |
| 29 | **NEW, rev 44 — THE BIGGEST FINDING OF THE REVISION, and it is LIGHTING, not livery.** The body red **renders twice as light as the photograph** and it is what makes the script illegible, the Calidad orange and the hubcap pink | **open.** Red / cream luminance, its own frame so exposure cancels: **model 0.504** against **ref_side 0.295** and **ref_nolita_doorshut 0.210** — two independent frames, different cameras and eras. Red G/R: **model 0.447** against 0.280 and 0.202. **BUT `RED`'s albedo is sRGB (196,49,36), G/R 0.250 — which sits BETWEEN the two references.** So the livery is right and §0.2's "not a deep crimson" stands; **the RENDER turns a 0.250 albedo into a 0.447 image.** Not exposure either — a brighter rig scales red and cream together and preserves the ratio; this ratio is not preserved. It is **ambient/bounce lift** off the studio cyclorama, which raises a dark surface proportionally far more than a light one. **Weathering is NOT the lever and was tested:** `paint` dust 1.0→0.30 moved red/cream 0.504→0.498. **This is a `studio.py` lighting pass**, and it is the single change that would most improve the hero. |
| 30 | **NEW, rev 44 — HIS REPORT, CONFIRMED.** The "Señor Tacombi" script is not legible enough | **open, and it is finding 29's symptom, not its own defect.** Script-ink to body-red luminance contrast: **model 1.734, reference 2.748** (Michelson 0.269 against 0.466) — the reference is **58 %** higher. The ink is fine; the background is too light. Model ink 190 DN on red 109; reference ink 122 on red 44. **Fix 29 and this follows.** |
| 31 | ~~**HIS REPORT** — the front bumper overlaps the cab door~~ | **CLOSED, rev 44.** Measured on the built mesh: `bumper_f` ran x **1.7350..2.1403** against a door aperture starting at **1.8171** — **82 mm of bumper inside the door**, with 89 of its 91 vertices in that span sitting **above** the door's own bottom outline, worst by **114 mm**. `1.735` was a bare literal and SPEC §10.72 had already struck the bumper's x extents as **UNMEASURED**. Now **expressed from the door it must not touch** (`max(DOOR_GAP_S x) + 10 mm` → 1.8273), so it cannot drift again. Wrap 373 → 281 mm, stated not tuned. Guards 0/0 both levels. |
