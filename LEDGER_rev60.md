...
|---|---|
| render, before | **0.545** |
| render, pan on `M["dark"]` | **0.352** [STALE — see below] |
| render, pan on `M["underseal"]` | **0.219** [STALE — see below] |
| photograph `ref_side.jpg` | **0.057** |

`probe_rev45_ground.py`, its own frame, ablation against built:

```
T1_NOUNDER=1   G3 0.8375   G4 0.5475     [STALE]
built          G3 0.7714   G4 0.2519     [STALE]

  ^^ SUPERSEDED AT REV 60c.  Both arms were measured on a mesh with the pan
     placed half its own depth low; the rev-60b repair raised the floor 45 mm
     and the gate was never re-run.  Re-measured:

T1_NOUNDER=1   G3 0.8376   G4 0.5475
built          G3 0.7935   G4 0.3602     <- the live figure
T1_UNDER_VIS=0.145 (the photographed ceiling band's top)   G4 0.2581
photograph ref_side.jpg                                    G4 0.057

     And that third row is why 0.2519 looked good: the BUGGED pan hung
     0.134-0.145 m low, so rev 60 was unknowingly measuring the deep-pan
     case.  Rev 60's "improvement" was substantially the bug.  See F120.
```
...
0.5475 before it was watched passing at 0.2519** (rule 3). **Re-watched at rev 60c: 0.5475 ablated, 0.3602 built.**
...
probe_rev45_ground 5 checked, 0 FAILED -- G4 0.2519 (was 0.5475 ablated)  [STALE: rev 60c reads 0.3602 / 0.5475]
```
