...
**`verify_clone.sh` WAS RUN ON THE ACTUAL HANDOFF COMMIT, not on the tree later.** ALL 272 PASS.
...
verify_clone.sh       ALL 272 PASS on a clean tree, AT THE HANDOFF COMMIT
                      <- 0 FIDELITY, 272 SELF-CONSISTENCY.  ONE row was added this
                         revision: the T1_SENOR_TARNISH companion row
...
evidence about the vehicle. **Not one of those 272 rows compares the model to a photograph.**
...
DROPPING.** `verify_clone.sh` ends **ALL 272 PASS** and its own verdict block says what that is
worth: **0 FIDELITY, 272 SELF-CONSISTENCY.**
...
./verify_clone.sh       # ALL 272 PASS -- and read its verdict block
```
