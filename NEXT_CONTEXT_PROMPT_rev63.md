...
**`verify_clone.sh` WAS RUN ON THE ACTUAL HANDOFF COMMIT, not on the tree later.** ALL 285 PASS.
...
verify_clone.sh       ALL 285 PASS on a clean tree, AT THE HANDOFF COMMIT
                      <- 0 FIDELITY, 285 SELF-CONSISTENCY.  THREE rows added this
...
DROPPING.** `verify_clone.sh` ends **ALL 285 PASS** and its own verdict block says what that is
worth: **0 FIDELITY, 285 SELF-CONSISTENCY.**
