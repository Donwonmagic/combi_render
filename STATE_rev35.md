...
| | |
|---|---|
| generated | 2026-08-17 01:42:40 UTC |
| git commit | `0ecbe4f` |
| git subject | rev 35: SPEC 10.89 -- the harmonic-conjugate route retired on a MISSING FEATURE (its third input is unobserved); u_lamp_far = 236 found to have no recorded derivation; the replacement bound's SIGN survives but its MAGNITUDES are withdrawn after rev 35's own adversarial audit found the collinearity precondition violated by the build's own hoop constants. Rule earned: checking the precondition you were warned about is not checking the preconditions. |
| working tree | clean |
| blender | 4.5.3 LTS |
