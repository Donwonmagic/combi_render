...
| `OPEN_FINDINGS.md` | the register. **F330–F344 are rev 77's; THE NEXT FREE ID IS F345 — DERIVE IT, do not read it here:** `grep -oE '\*\*F[0-9]+\*\*' OPEN_FINDINGS.md | sort -uV | tail -1`. It outranks prose. ⚠ **This row said "F330–F336" while the same brief leaned on F337, F338, F340 and F341; that is exactly how F341 came to be assigned TWICE (see F344). Corrected (rule 13) — COUNT THE REGISTER, do not trust this row either** |
