# NEXT CONTEXT PROMPT — rev 15

Please act as my expert. Continue the Señor Tacombi combi build. **Fourteen
revisions sit behind this.** You are picking up mid-stream, not starting.

## Step 1 — read my memory BEFORE you read any code
`/areas/tacombi-combi-3d.md`, then `/areas/tacombi-combi-sticker.md`, then
...
See §1 below. Anything not on my disk does not exist.
...
```
Report their ACTUAL output. Both levels, every time — "guards green" was true
only at SUB=1 for six revisions while the production build silently lost both
cab-door shut lines.

## Step 4 — read, in this order
`STATE.md` → `SPEC.md` §10, then §10.9 through §10.30i → this file →
...
This has now paid off six times. **And my answers are not infallible — ask me
...
**FIRST QUESTION FOR ME, before any new photograph is measured:** the
green/red question in §5.

## Step 6 — the work
§6 below is the ordered list. Order matters and it is not the order of
severity — it is chosen so nothing later invalidates something earlier.
Batch all changes into one rebuild. Re-run both guards after.

## Step 7 — raise the resolution
rev 12 shipped 3000×2000; rev 13 went to 3600×2400. Go higher again. Drive
...
`optics-6` CLOSED as refuted — **the vehicle does not float, do not add a shadow
it does not cast.** `materials-5` CLOSED as dead. The tail lamp is **round**.
**The windscreen is a SPLIT screen, two panes** (settled rev 14).
...
* **Check what a guard, or a probe, or a crop can physically SEE.**
* **A guard can be strengthened instead of widened.**
* **A negative control turns a number into a measurement** (rev 14: 2.1 % meant
  nothing until `T1_W_ART=0` made it 0.05 points).
