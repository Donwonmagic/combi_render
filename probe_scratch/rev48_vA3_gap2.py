...
def bands(ty, Z, lo=-55.0, hi=10.0):
    best=None
...
def run(f, box, Z=8, tag="", R0=80, rg=35, mxT=150, satT=0.30, openfrac=0.12, save=None):
    ...
    ys,xs=np.nonzero(burst); W=(xs.max()-xs.min()+1)
    core=nd.binary_opening(burst, disk(int(openfrac*W)))
    lab,n=nd.label(core)
    if n>1: core=lab==(int(np.argmax(nd.sum(core,lab,range(1,n+1))))+1)
    ty=nd.binary_opening(core & (mx>mxT)&(sat<satT), np.ones((5,5)))
    print("%-12s burst %.1f x %.1f  core %.1f x %.1f  typepx %d" % (tag,W/Z,(ys.max()-ys.min()+1)/Z,
        (np.nonzero(core)[1].ptp()+1)/Z,(np.nonzero(core)[0].ptp()+1)/Z, ty.sum()))
    r=bands(ty,Z)
    print("             ",("gap/cap %.3f  ang %.1f  cap %.2f px  gap %.2f px"%r) if r else "NOT SEPARABLE")
    if save:
        out=np.zeros(burst.shape+(3,),np.uint8)
        out[...,0]=burst*90; out[...,0][core]=160; out[...,1]=ty*255; out[...,2]=ty*255
        Image.fromarray(out).resize((im.width*3,im.height*3)).save(save)
    return r

run("IMG_2073.jpeg",(1108,360,1210,445),tag="2073 GREEN",save="probe_scratch/rev48_vA3_m2_2073.png")
run("ref_side.jpg",(728,300,845,392),tag="side RED",save="probe_scratch/rev48_vA3_m2_side.png")
run("ref_rear34.jpg",(748,288,862,378),tag="rear34 RED",mxT=170,save="probe_scratch/rev48_vA3_m2_rear34.png")
