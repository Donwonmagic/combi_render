...
nohup setsid env T1_SUB=1 T1_PREVIEW=front,side,hero,hero34r T1_PFX=r68 T1_RX=1600 T1_RY=1100 \
  T1_SAMP=96 /tmp/blender/blender -b -P build.py > /tmp/r68.log 2>&1 < /dev/null &
```
...
**`grep -c Saved: /tmp/r68.log` must be 4** — a backgrounded runner's exit code is the
redirect's. **USE `setsid`, NOT A BARE `nohup &`** (F173).

**AND CHECK YOUR CLONE IS THE TIP.** Rev 62 through **rev 67** all arrived **SHALLOW** —
**ten running**. A content check cannot detect that you are on an old commit of the same
repository. **Only `git fetch --unshallow` and the ahead/behind loop in §1 find it.**

**THEN RUN `./judge_set.sh r68`.** `post.py` implements bloom → CA → vignette → grain and the
...
**READ `LEDGER_rev67.md` §6 AND §4 BEFORE YOU PLAN** — what rev 67 did NOT do, and the four
...
> *[owner, rev 66, asked directly whether rev 67 should do the nose or finish the emblem]*
> **the NOSE.**

**THE NOSE'S PLAN BULGE IS MEASURED FOR THE FIRST TIME (F207).** On the mesh,
`x(y=0) − x(|y|=0.70)` — the forward convexity of the whole face:

```
    SHIPPED               z=0.65 +19.6   z=0.80 +19.6   z=0.95 +20.0   z=1.10 +16.3  mm
    T1_NOSE_BULGE -> 0    z=0.65  +6.2   z=0.80  +6.2   z=0.95  +6.2   z=1.10  +2.4  mm
    13.4 mm of the 19.6 is nose_shape()'s constant;  6.2 mm is the LOFTED SHELL itself
```

**AND THE STRUCTURAL REASON FIFTEEN REVISIONS NEVER TOUCHED THIS AXIS.** In a side elevation
the silhouette at each height is **max-over-y of x**, which for a plan-convex nose is
...
down to the V apex**, so that edge is the **PAINT REGION's** silhouette. Caught by painting,
then by the mesh disagreeing by a factor of seven. The corrected result is a real negative:

```
    sub-pixel sagitta of the leading edge, whole-window quadratic fit
      MODEL out/r67_side.png +0.24 px | ref_side.jpg -1.21 | doorshut -0.29
    THE VERTICAL SECTION IS STRAIGHT TO ~1 px IN ALL THREE.  Not the defect.
```

*(And two frame facts: **`ref_side.jpg` has the CAB DOOR OPEN** and occludes the nose to a
sliver; **`ref_nolita_doorshut.jpg` has it SHUT** and is the unambiguous side elevation for
nose geometry, at 107 px/m against `ref_side`'s 220.)*

---
## §0.065 AND THE ADVERSARY FOUND FOUR INSTRUMENTS REPORTING THINGS THAT ARE NOT MEASUREMENTS

**F208 — `T1_VW_CAPMIN` HAS BEEN A COMPLETE NO-OP SINCE REV 66, AND THREE REGISTER ROWS REST
ON IT.** Watched, fresh process per configuration:

```
    []                shipped   residual 0.0755   cells [5167, 5122, 2277, 2081, 2023, 1887]
    [T1_VW_CAPMIN=1]  IDENTICAL residual 0.0755   cells [5167, 5122, 2277, 2081, 2023, 1887]
    [T1_VW_NOARC=1]   DIFFERS   residual 0.0986   cells [4845, 4791, ...]
```

F202's arc cut removed the end caps from `_drive`, which is what CAPMIN acted on — **and
F202's own register row says so. It never reached §2 or §6.** **It voids §2 row 1 (F101,
"cells 6 → 2" — today 6), §2 row 15 (F199) and F196's live text.** **AND THE REV-67 BRIEF
NAMED THE WRONG ROWS AS UNSAFE: F179 never used C6 and it re-runs and STANDS.**

**F209 — F200 IS A RE-DISCOVERY OF F139, `MEASURED-rev61`.** *"C6's TARGET OF 7 IS
CONTAMINATED, AND 7 IS TOPOLOGICALLY UNREACHABLE ANYWAY … The photograph's genuine glyph count
is 6, the same as the build."* Five revisions early. **C6's re-base 7 → 6 STANDS on its
photograph-side evidence. What falls is *"the mark CANNOT make seven"*** — 7 appears in 3 of
144 builds under F200's own protocol, and F103 and F174 both said so already.

**F210 — C6's REACH FIGURES ARE THE FIT TARGET RESTATED.** `1 − 0.8 × (0.028/0.140) = 0.84`
**exactly**; the 108 vertices are the arc cut's own tessellation. **Under `T1_VW_NOARC` the
same line prints "8 of 16".** **Do NOT treat "the mesh says every terminal reaches" as an
independent observation contradicting the render.**

**F211 — C10 PRINTS A `9.9` SENTINEL AS A LANDMARK MOVE.** Landmarks are fractions in [0,1].
At 276 rows `landmarks()` returns `None`, so nothing moved and the fallback fired.

**F212 — BUT F205 ITSELF RE-MEASURES AND STANDS, ON THIS REVISION'S OWN RENDER.**
Photograph **6** interior cells; `out/r67_front.png` **4 / 3 / 2** at ink thresholds
20 / 30 / 40, stable under crop padding. **The emblem's render-side gate is RED and it is the
owner's own second sentence.**

---
## §0.07 THE MACHINE'S VERDICT AT CLOSE OF REV 67 — every one watched print
...
probe_rev46_vw.py         12 checked, 1 FAILED -- C4 ONLY, at 0.0755 against a bar of
                          0.045.  BUT SEE F210/F211: C6's reach figures and C10's
                          evidence line are not measurements.
...
F205 ON THE FRAME (F212)  PHOTOGRAPH 6  |  out/r67_front.png 4 / 3 / 2 at thr 20/30/40
                          <- the gate that matters, and it is RED
...
audit.py T1_SUB=2         VERIFY: 0 fail, 0 warn -- STATE.md regenerated, and it moved
                          ONLY IN PROVENANCE.  Every vertex count and dimension
                          identical, which is the evidence the change was contained
probe_rev65_unproject.py  10 checked, 0 FAILED
probe_rev64_shear.py      6 checked, 0 FAILED
probe_rev63_trace.py      ALL CONTROLS PASS -- but FIVE of its ten are ctl(..., True, ...)
                          and cannot fail.  Do not quote its total as a verdict
probe_rev63_reach.py      ALL CONTROLS PASS -- and it now reports SIX ring contacts at
                          16, 66, 114, 163, 248, 292 deg.  **F180's live text says FOUR
                          at 62/118/212/328 and is STALE**
...
probe_rev59_nose.py       5 checked, 0 FAILED -- and it STILL does not measure the nose's
                          SECTION.  That is now probe_rev67_nose.py's job (F207)
...
DROPPING.** `verify_clone.sh` ends **ALL 336 PASS**: **0 FIDELITY, 336 SELF-CONSISTENCY.**
...
./verify_clone.sh       # ALL 336 PASS -- and read its verdict block
```

**MEASURED AT REV 67 PICKUP:** the clone was **SHALLOW** — the **TENTH running**, and
`bootstrap.sh` unshallowed it to 614 commits;
`fetch --prune` printed `- [deleted] origin/claude/rev67-bootstrap-nose-jwawdm`, **the
designated branch, before anything had been pushed to it, THE TENTH RUNNING**; HEAD was
**0 ahead / 0 behind** `origin/main`; **row 9 PASSED**; and
`git diff --name-only HEAD...origin/main` was **EMPTY** — no new photographs.
**EXPECT THE BRANCH DELETION AGAIN AT REV 68.**
...
rev 55; the pickup figure is NOT the close figure. **AT REV 67's CLOSE, see `LEDGER_rev67.md` §7.**
...
**WHAT IS IN THE TREE AT REV 67's CLOSE (unchanged by rev 67 -- the nose took the revision):** rev 63's six spine constants (`VW_V_TIP_X 0.3287`,
...
reach            T1_VW_CAPMIN            cells 6 -> 2                       (F101)
                                         *** DEAD AT REV 66, FOUND AT REV 67:
                                         T1_VW_CAPMIN IS A NO-OP.  Its run is
                                         BIT-IDENTICAL to shipped and gives
                                         6 cells, NOT 2.  F202 removed the end
                                         caps from `_drive`, which is what it
                                         acted on.  UNREPRODUCIBLE.      (F208) ***
...
T1_VW_CAPMIN + T1_VW_PUREFIT             6 cells / 2.24.  TRIED, REFUTED    (F199)
                                         *** UNSAFE: CAPMIN contributes NOTHING
                                         (F208), so this pair is PUREFIT alone
                                         and today reads 6 cells / 2.615.  And
                                         NOARC+CAPMIN CRASHES.            (F208) ***
...
> **AND A WARNING ABOUT THIS LIST ITSELF — CORRECTED AT REV 67, BECAUSE THE REV-67 BRIEF
> NAMED THE WRONG ROWS.** It said the three unsafe rows were F152, F103 and **F179**.
> **F179 NEVER USED C6** — it is `probe_rev63_reach.py`'s contact count, it was RE-RUN at
> rev 67, and **it STANDS.** The rows that are actually unsafe are:
>
> * **F101, F199 and F196** — all three scored on **`T1_VW_CAPMIN`, which has done NOTHING
>   AT ALL since rev 66 (F208).** Their figures are unreproducible today.
> * **F152** — scored on C8's old target and a 96 %-broken residual. **Already superseded and
>   SHIPPED AGAINST at rev 66 (F204).**
> * **F103** — its *residual* half is invalidated by F203; **its "7 cells IS reached" half is
>   sound, and it REFUTES F200's unreachability leg (F209).**
>
> **AND THE LIST IS SEVENTEEN ROWS, NOT THE "14" THE REV-67 BRIEF CLAIMED.** Count it.
> **A refutation inherits its instrument. Before you accept any row here, check what it was
> measured with — and CHECK THE REGISTER FOR THE FINDING BEFORE ACCEPTING ANYTHING AS NEW**,
> which is how F139 was found sitting under F200 (F209).

---
## §3 THE WORK LIST FOR REV 68
...
2. **F205/F212 — THE RENDER CUTS THREE INTERIOR CELLS WHERE THE PHOTOGRAPH CUTS SIX.**
   ...
   AND IT IS THE OWNER'S OWN SENTENCE.** **BUT READ F210 FIRST:** the "mesh says 0.8400 R,
   102 of 108 vertices in the band" half of rev 66's *"measured contradiction"* is
   `1 − 0.8 × _BAND_FRAC` restated plus the arc cut counting its own tessellation. **Under
   `T1_VW_NOARC` the same line prints "8 of 16".** There is no contradiction to resolve —
   there is a render-side defect to fix. Start from `probe_scratch/rev67_render_cells.png`.
3. **C4 IS THE EMBLEM'S LAST RED ROW — 0.0755 against a bar of 0.045.** `T1_VW_SOLVE=1` is
   STILL sitting unrun, two revisions on. Errors now: **L1 −0.0307, L3 −0.0239, L4 +0.0634**.
   **RE-SOLVE THE SIX CONSTANTS AND THEN RENDER IT** — rule 41.
4. **REPAIR OR RETIRE `T1_VW_CAPMIN` (F208).** It has done nothing since rev 66 and three
   register rows and two lines of §2 rest on it. **An ablation that no longer ablates reads
   as "no effect ⇒ not the lever" — the exact laundering the switches exist to stop.** Either
   make it act on the arc-cut terminals, or make it REFUSE loudly (rule 37). `NOARC+CAPMIN`
   currently **crashes** with `AssertionError`, which is a crash, not a refusal.
5. **FIX C10's `9.9` SENTINEL (F211) AND RE-GRADE F180 (stale: it says four ring contacts,
   `probe_rev63_reach.py` reports six).** Both are one-line honesty repairs.
6. **THE FIT DEPTH IS STILL UNMEASURED.** The glyph's extreme is fitted **20 % into the band**
   (`1.0 - 0.8 * _BAND_FRAC`). Rev 44 warns that fitting to the OUTER radius buries the arms.
   **The answer is between and it is a MEASUREMENT, not a guess.**
7. **PUT AN ADVERSARY ON THIS BRIEF (rule 15), AND *DISPATCH* IT — POINT IT AT §2's REFUTED
   LIST AGAIN.** It paid at rev 64 (fourteen), and at rev 67 (thirteen, five confirmed by
   hand). **Tell it to check the REGISTER for the finding before accepting that anything is
   new** — that is how F139 was found under F200.
8. **FIX `probe_rev63_shapefit.py`** — stale baseline, and it reads `CAP_EMBLEM_WFRAC`, the
   HUBCAP's weight, where the nose ships its own (F178's trap).
9. **F156 — `flank_compare`'s `Senor` row scores a DELIBERATE DEPARTURE. SEVEN revisions
   unacted.** Re-base the reference or annotate the row.
10. **TRIAGE `REMAINING_WORK_rev61.md` §I** — 27 rows, SEVEN revisions.
11. **THE SURVIVING PANEL ITEMS**, untouched for seven revisions: the glass is a flat slab
...
12. **F143 — TWO LOUDSPEAKERS STAND ON THE ROOF AND ARE UNMODELLED.** 55 revisions.
13. **THE INHERITED CLUSTER** — F14 (**fifteen** revisions un-re-measured), F15, F10, F20.
14. **`delivery/READ_ME_FIRST.txt` LISTS THE MODEL'S KNOWN DEFECTS TO HIM.** It does not yet
    mention the rev-63 emblem change, F184, F194, F197, rev 66's F200/F202/F204, or
    **rev 67's F207**.
...
out/r68_hero.png` — **and PASS IT A `.png`**, or it globs `out/` by mtime and reproduces F132.
...
>
> **⚠ CORRECTED AT REV 67 (F209). THE GENERALISATION IS WRONG, AND THE CASE IT WAS DRAWN FROM
> IS NOT WHAT IT LOOKED LIKE.** `OPEN_FINDINGS.md` ALREADY CARRIED THE FINDING, as **F139**,
> at grade **`MEASURED-rev61`**, five revisions before F200: *"C6's TARGET OF 7 IS
> CONTAMINATED, AND 7 IS TOPOLOGICALLY UNREACHABLE ANYWAY … The photograph's genuine glyph
> count is 6, the same as the build."* **It was not invisible. It was written down, graded
> MEASURED, and READ PAST by five revisions.** And the "cannot ever produce" half is itself
> refuted — 7 appears in 3 of 144 builds under F200's own protocol, and F103 and F174 both
> reported it earlier. **Read rule 45 as a rule about GRADE DECAY AND UNREAD CARRIERS —
> which §8 already says is itself a finding — not about discovery.**
...
> **47. NEW AT REV 67 — AN ABLATION SWITCH CAN STOP ABLATING, AND SILENCE IS ITS FAILURE
> MODE.** `T1_VW_CAPMIN` has been a **complete no-op since rev 66** — its run is bit-identical
> to shipped — because F202 removed from `_drive` the very terminals it acted on. Three
> register rows and two lines of §2 still rest on figures it can no longer produce. **An
> ablation that no longer ablates reads to the next agent as "no effect ⇒ not the lever",
> which is precisely the laundering the switches exist to stop.** **Ablate every switch you
> rely on and check the output ACTUALLY MOVED** — and when you remove what a switch acted on,
> retire the switch in the same edit (F208).

> **48. NEW AT REV 67 — AN ACCEPTANCE BAR EXPRESSED AS A FRACTION OF ITS OWN SPAN CANNOT
> REFUSE A LONG ENOUGH INPUT.** Rev 67 wrote *"a traced edge must have rms ≤ 12 % of its
> span"* and watched a whole-frame column scan — **rms 61.85 px on 831 px, catching the
> counter nosing and the tail board as well as the bumper** — sail through it at 7.4 %. A
> smooth boundary sits within a few PIXELS of a parabola however long it is. **Re-cut as
> `max(4 px, 3 % of span)`, and that arithmetic is now a verifier row so it cannot loosen
> again.** Rule 3: a bar that cannot refuse is not a bar.
...
`T1_VW_CAPMIN` (**DEAD -- A COMPLETE NO-OP SINCE REV 66; bit-identical to shipped.  F208**),
`T1_VW_PUREFIT`, `T1_VW_CELLSOLVE`, `T1_VW_DUMP`, `T1_VW_RES`, `T1_VW_WSWEEP`,
...
TOGETHER**), `T1_VRISE`, **`T1_NOSE_BULGE` (NEW at rev 67 -- scales the nose's PLAN BULGE,
`t1_shell.NOSE_BULGE`; `probe_rev67_nose.py`'s M2 kill, WATCHED FIRING 19.6 -> 6.2 mm.  F207)**, `T1_DOOR_STALE`, `T1_NORIG`, `T1_RIG`, `T1_WORLD`, `T1_MOT_AMP`,
...
**Rev 67 added F207–F213**, of which **four are defects in this project's OWN instruments**
and one — **F209** — is the register catching itself: a row it already held, at grade
`MEASURED-rev61`, that five revisions read past. **Rev 66 added F200–F206**, of which **three are defects in this project's OWN instruments**
...
**STILL INHERITED AND OLDEST:** **F14** (`gal_end_f`'s sight lines, **rev 52 — FIFTEEN
...
**AND `REMAINING_WORK_rev61.md` §I IS STILL NOT TRIAGED** — 27 rows, **seven revisions**.

---
## §9 THE HORIZON BEYOND REV 68
...
**AND WHAT REV 67 LEARNED LAST, WHICH RE-RANKS THE REST.** The register caught itself: the
finding rev 66 spent a revision discovering had been sitting in `OPEN_FINDINGS.md` at grade
`MEASURED-rev61` for five revisions (F139/F209). **Before you derive anything, READ THE
REGISTER FOR IT.** And an ablation switch was found to have stopped ablating a whole revision
earlier, silently, with three register rows still resting on it (F208).

**WHAT MOVED AT REV 67.** **The nose got an instrument for the first time** — its plan bulge
measured at **19.6 mm**, the ablated shell's own convexity separated out at **6.2 mm**, and
the structural reason no side elevation could ever have seen it. **Four of the emblem's
instruments were found reporting things that are not measurements** (F208, F210, F211, and
C10's sentinel), **and F205 was re-measured on the frame and STANDS** (F212). **What did NOT
move: `NOSE_BULGE` itself — the photographs cannot say what to move it to — and C4.**

| horizon | the work | why |
|---|---|---|
| **next** | **ASK HIM THE NOSE A/B (§3 item 1, F207)** | **The instrument exists now and the photograph is CEILED.** `T1_NOSE_BULGE` makes the A/B one command — show him two crops, not a number. It is the only thing that unblocks the nose |
| **next** | **F205/F212 — THE RENDER CUTS 3 CELLS WHERE THE PHOTOGRAPH CUTS 6 (§3 item 2)** | **The owner's own second sentence, RE-MEASURED at rev 67 on `out/r67_front.png` and still RED.** But read F210 first: the "mesh says 0.8400 R" half was never an independent observation |
| **next** | **C4 — re-solve the six constants (§3 item 3)** | The emblem's last red row, **the owner's first sentence**, and `T1_VW_SOLVE=1` has now sat unrun for TWO revisions |
| **next** | **REPAIR OR RETIRE `T1_VW_CAPMIN` (§3 item 4, F208)** | **It has done nothing since rev 66** and three register rows and two lines of §2 rest on it. `NOARC+CAPMIN` crashes rather than refusing |
| **next** | **F211's `9.9` sentinel, and F180's stale contact count** | Two one-line honesty repairs in gates the briefs quote |
| **next** | **THE TIP/RING GAP (§3 item 6)** | The photograph shows none; nobody has measured the fit depth |
| **next** | **AN ADVERSARY, DISPATCHED, ON §2's REFUTED LIST** | It paid at rev 64 (fourteen) and rev 67 (thirteen). **Tell it to check the REGISTER before accepting anything as new** |
| **near** | **F156 — the `Senor` gate row scores a DEPARTURE** | SEVEN revisions unacted |
| **near** | **Fix `probe_rev63_shapefit.py`** | Stale baseline AND the hubcap's weight — **and F204 moved the nose's** |
| **near** | **Triage `REMAINING_WORK_rev61.md` §I** | 27 rows, seven revisions |
| **near** | **Glass, tyres, the tail's barrel, the shut lines** | Untouched for seven revisions |
| **near** | **F143 — the roof loudspeakers** | Unmodelled since rev 12 — 55 revisions |
| **LOWERED** | **F192 — prove the large-format chain** | **He ruled the MODEL comes first (F193).** Do not spend a revision on the pipeline |
| **then** | **F10–F14 — the galley cluster** | F14 is FIFTEEN revisions inherited |
